...
;; Resolved Outcomes
(define-map time-outcomes uint uint)
...
(define-public (place-yield-bet (target-time uint) (amount uint) (prediction uint))
    (begin
        ...
        (let
            (
                ...
                (current-pool (default-to { total-pool: u0, even-pool: u0, odd-pool: u0, status: "open" } (map-get? time-pools target-time)))
                (user-credits (default-to u0 (map-get? user-yield-credits tx-sender)))
            )
            ;; Must bet at least 2 blocks in the future
            (asserts! (> target-time (+ current-block u1)) ERR-ROUND-CLOSED)
            ...
            (asserts! (is-none (map-get? stakes { target-time: target-time, user: tx-sender })) ERR-STAKE-EXISTS)
            
            ;; Deduct yield credits from user
            (map-set user-yield-credits tx-sender (- user-credits amount))
            
            ;; Update stakes map
            (map-set stakes { target-time: target-time, user: tx-sender } { amount: amount, prediction: prediction })
            
            ;; Update block pool stats
            (map-set time-pools target-time {
...
(define-public (resolve-pool (target-time uint))
    (let
        (
            (current-time stacks-block-time)
            (pool (unwrap! (map-get? time-pools target-time) ERR-ROUND-NOT-FOUND))
        )
        (asserts! (>= current-time target-time) ERR-TOO-EARLY)
        (asserts! (is-eq (get status pool) "open") ERR-ALREADY-CLAIMED)
        
        ;; Determine outcome based on current timestamp parity (1 for Even, 2 for Odd)
        (let
            (
                (outcome (if (is-eq (mod current-time u2) u0) u1 u2))
            )
            (map-set time-outcomes target-time outcome)
            (map-set time-pools target-time (merge pool { status: "resolved" }))
            (ok outcome)
        )
    )
)
...
(define-public (claim-reward (target-time uint))
    (let
        (
            (stake (unwrap! (map-get? stakes { target-time: target-time, user: tx-sender }) ERR-NO-STAKE))
            (pool (unwrap! (map-get? time-pools target-time) ERR-ROUND-NOT-FOUND))
            (outcome (unwrap! (map-get? time-outcomes target-time) ERR-ROUND-CLOSED))
        )
        ;; Asserts
        (asserts! (is-eq (get status pool) "resolved") ERR-ROUND-CLOSED)
        (asserts! (is-eq (get prediction stake) outcome) ERR-NOT-WINNER)
        (asserts! (is-none (map-get? claimed-users { target-time: target-time, user: tx-sender })) ERR-ALREADY-CLAIMED)
        ...
        (let
            ...
            ;; Mark claimed
            (map-set claimed-users { target-time: target-time, user: tx-sender } true)
...
(define-read-only (get-time-pool (target-time uint))
    (map-get? time-pools height)
)

(define-read-only (get-user-stake (target-time uint) (user principal))
    (map-get? stakes { target-time: height, user: user })
)

(define-read-only (get-outcome (target-time uint))
    (map-get? time-outcomes height)
)

(define-read-only (has-user-claimed (target-time uint) (user principal))
    (default-to false (map-get? claimed-users { target-time: height, user: user }))
)
